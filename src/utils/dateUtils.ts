...
/**
 * Formata uma string ISO 8601 (ou objeto Date) para o padrão brasileiro.
 * Função flexível que aceita um padrão de formatação personalizado.
 * Ex: formatToBrazilianDate('2025-12-04T10:30:00.000Z', 'dd/MM/yyyy HH:mm') -> '04/12/2025 07:30'
 * @param dateString A string de data/hora ISO 8601 ou objeto Date.
 * @param formatPattern O padrão de formatação desejado (padrão: 'dd/MM/yyyy HH:mm').
 * @returns A data formatada no padrão especificado.
 */
export function formatToBrazilianDate(dateString: string | Date, formatPattern: string = 'dd/MM/yyyy HH:mm'): string {
  try {
    const date = typeof dateString === 'string' ? parseISO(dateString) : dateString;
